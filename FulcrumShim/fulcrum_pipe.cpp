...
bool fulcrum_pipe::ConnectOutputPipe()
{
	...
	{
		dtDebug(_T("%.3fs :: FULCRUM PIPE 1 (OUTPUT PIPE) COULD NOT BE OPENED!\n"), GetTimeSinceInit());
...
bool fulcrum_pipe::ConnectInputPipe()
{
	...
	{
		dtDebug(_T("%.3fs :: FULCRUM PIPE 2 (INPUT PIPE) COULD NOT BE OPENED!\n"), GetTimeSinceInit());
...
void fulcrum_pipe::ShutdownPipe()
{
	...
	dtDebug(_T("%.3fs    Closed output pipe for fulcrum correctly!\n", GetTimeSinceInit()));
	Loaded = false;
}
